...
class LeafController():
    ...
    def init_data(self):
        ...
        self.wid_port_mapping = {0:151, 1:150, 2:149, 3:148, 7:132, 100: 152}
        self.port_mac_mapping = {132: 'F8:F2:1E:3A:13:EC', 148: 'F8:F2:1E:3A:13:C4', 152: 'EE:AA:AA:AA:AA:EE', 151: 'F8:F2:1E:3A:13:C4',  150: 'F8:F2:1E:3A:13:C4', 149:'F8:F2:1E:3A:13:C4'}
...
class SpineController():
    ...
    def init_tables(self):
        ...
        self.forward_falcon_switch_dst.info.key_field_annotation_add("hdr.falcon.dst_id", "id")
        ...
        self.adjust_random_range_ds.info.key_field_annotation_add("falcon_md.cluster_num_valid_queue_signals", "num_valid_ds")
        self.get_rand_leaf_id_1 = bfrt_info.table_get("SpineIngress.get_rand_leaf_id_1")
        self.get_rand_leaf_id_1.info.key_field_annotation_add("falcon_md.random_ds_index_1", "rand_idx_1")
        self.get_rand_leaf_id_2 = bfrt_info.table_get("SpineIngress.get_rand_leaf_id_2")
        self.get_rand_leaf_id_2.info.key_field_annotation_add("falcon_md.random_ds_index_2", "rand_idx_2")
        # self.adjust_random_range_us = bfrt_info.table_get("LeafIngress.adjust_random_range_us")
        # self.adjust_random_range_us.info.key_field_annotation_add("falcon_md.cluster_num_valid_us", "num_valid_us")
        # self.get_spine_dst_id = bfrt_info.table_get("LeafIngress.get_spine_dst_id")
        # self.get_spine_dst_id.info.key_field_annotation_add("falcon_md.random_id_1", "random_id")
    ...
    def init_data(self):
        ...
        self.initial_idle_list = [0, 1, 2, 3]
        #self.initial_idle_count = len(self.initial_idle_list)
        self.initial_idle_count = 3
        self.intitial_qlen_state = [0] * len(self.initial_idle_list)
        self.wid_port_mapping = {0:36, 1:44, 2:20, 3:52, 4:28, 5:20, 7:56}
        #self.port_mac_mapping = {20: 'F8:F2:1E:3A:13:EC', 28: 'F8:F2:1E:3A:13:C4'}
        
        self.qlen_unit = [0b00000001] * len(self.initial_idle_list) # assuming 3bit showing fraction and 5bit decimal: 0.125 (8 workers in each rack)
        
        self.intitial_deferred_state = [0] * len(self.initial_idle_list)
        self.num_valid_ds_elements = 4 # num available leaves this vcluster (the number in hardware will be 2^W)

        self.leaf_start_idx = self.TEST_VCLUSTER_ID * self.MAX_VCLUSTER_WORKERS
    ...
    def set_tables(self):
        ...
        for wid in self.wid_port_mapping.keys():
            self.forward_falcon_switch_dst.entry_add(
                self.target,
                [self.forward_falcon_switch_dst.make_key([client.KeyTuple('hdr.falcon.dst_id', wid)])],
                [self.forward_falcon_switch_dst.make_data([client.DataTuple('port', self.wid_port_mapping[wid])],
        ...
        for idx, leaf_id in enumerate(self.initial_idle_list):
            self.set_queue_len_unit_1.entry_add(
                self.target,
                [self.set_queue_len_unit_1.make_key([client.KeyTuple('falcon_md.random_id_1', leaf_id), client.KeyTuple('hdr.falcon.cluster_id', TEST_VCLUSTER_ID)])],
            ...
            self.set_queue_len_unit_2.entry_add(
                self.target,
                [self.set_queue_len_unit_2.make_key([client.KeyTuple('falcon_md.random_id_2', leaf_id), client.KeyTuple('hdr.falcon.cluster_id', TEST_VCLUSTER_ID)])],
            ...
            self.get_rand_leaf_id_1.entry_add(
                self.target,
                [self.get_rand_leaf_id_1.make_key([client.KeyTuple('falcon_md.random_ds_index_1', idx), client.KeyTuple('hdr.falcon.cluster_id', TEST_VCLUSTER_ID)])],
            ...
            self.get_rand_leaf_id_2.entry_add(
                self.target,
                [self.get_rand_leaf_id_2.make_key([client.KeyTuple('falcon_md.random_ds_index_2', idx), client.KeyTuple('hdr.falcon.cluster_id', TEST_VCLUSTER_ID)])],
        ...
        self.adjust_random_range_ds.entry_add(
            self.target,
            [self.adjust_random_range_ds.make_key([client.KeyTuple('falcon_md.cluster_num_valid_queue_signals', 2)])],
            [self.adjust_random_range_ds.make_data([], 'SpineIngress.adjust_random_leaf_index_1')]
        )
        self.adjust_random_range_ds.entry_add(
            self.target,
            [self.adjust_random_range_ds.make_key([client.KeyTuple('falcon_md.cluster_num_valid_queue_signals', 4)])],
            [self.adjust_random_range_ds.make_data([], 'SpineIngress.adjust_random_leaf_index_2')]
        )
        self.adjust_random_range_ds.entry_add(
            self.target,
            [self.adjust_random_range_ds.make_key([client.KeyTuple('falcon_md.cluster_num_valid_queue_signals', 16)])],
            [self.adjust_random_range_ds.make_data([], 'SpineIngress.adjust_random_leaf_index_4')]
        )
        self.adjust_random_range_ds.entry_add(
            self.target,
            [self.adjust_random_range_ds.make_key([client.KeyTuple('falcon_md.cluster_num_valid_queue_signals', 256)])],
