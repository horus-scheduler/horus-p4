...
class LeafController():
    ...
    def init_tables(self):
        ...
        self.register_egress_tstamp = bfrt_info.table_get("HorusEgress.egress_tstamp")
    ...
    def set_tables(self):
        for leaf_id in range(self.NUM_LEAVES):
            ...
            for i in range(self.num_valid_us_elements): # TODO: Make number of spines dynamic
                self.get_spine_dst_id.entry_add(
                    self.target,
                    [self.get_spine_dst_id.make_key([client.KeyTuple('horus_md.random_id_1', i), client.KeyTuple('hdr.horus.pool_id', leaf_id)])],
        ...
        self.forward_horus_switch_dst.entry_add(
                self.target,
                [self.forward_horus_switch_dst.make_key([client.KeyTuple('hdr.horus.dst_id', self.CPU_PORT_ID)])],
                [self.forward_horus_switch_dst.make_data([client.DataTuple('port', self.CPU_PORT_ID), client.DataTuple('dst_mac', client.mac_to_bytes(self.port_mac_mapping[self.CPU_PORT_ID]))],
                                             'LeafIngress.act_forward_horus')]
            )
    ...
    def set_tables(self):
        ...
        for idx, leaf_id in enumerate(self.initial_idle_list):
            self.set_queue_len_unit_1.entry_add(
                self.target,
                [self.set_queue_len_unit_1.make_key([client.KeyTuple('horus_md.random_id_1', leaf_id), client.KeyTuple('hdr.horus.pool_id', TEST_VCLUSTER_ID)])],
            ...
            self.set_queue_len_unit_2.entry_add(
                self.target,
                [self.set_queue_len_unit_2.make_key([client.KeyTuple('horus_md.random_id_2', leaf_id), client.KeyTuple('hdr.horus.pool_id', TEST_VCLUSTER_ID)])],
            ...
            self.get_rand_leaf_id_2.entry_add(
                self.target,
                [self.get_rand_leaf_id_2.make_key([client.KeyTuple('horus_md.random_ds_index_2', idx), client.KeyTuple('hdr.horus.pool_id', TEST_VCLUSTER_ID)])],
